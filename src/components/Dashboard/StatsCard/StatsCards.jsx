...
const StatsCards = () => {
  ...
  const derivedCards = useMemo(() => {
    const balanceValue = Number.isFinite(totalBalance) ? totalBalance : 0;
    const monthlySavingsValue = Number.isFinite(savedThisMonth)
      ? savedThisMonth
      : monthlyStats.income - monthlyStats.expense;
    const monthlySavingsIncome = Number(monthlySavingTotals?.income ?? monthlyStats.income ?? 0);

    return baseCardData.map((item) => {
      if (item.title === "Total Balance") {
        return { ...item, value: formatCurrency(balanceValue), positive: balanceValue >= 0 };
      }
      if (item.title === "Spent This Month") {
        return { ...item, value: formatCurrency(monthlyStats.expense), positive: monthlyStats.expense <= monthlyStats.income };
      }
      if (item.title === "Saved This Month") {
        const changePct = monthlySavingsIncome
          ? Number(((monthlySavingsValue / monthlySavingsIncome) * 100).toFixed(2))
          : item.change;

        return {
          ...item,
          value: formatCurrency(monthlySavingsValue),
          positive: monthlySavingsValue >= 0,
          change: changePct,
        };
      }
  ...
  }, [investmentsTotal, monthlySavingTotals, monthlyStats.expense, monthlyStats.income, savedThisMonth, totalBalance]);

  const derivedNumbers = useMemo(() => {
    const savingsRate = monthlyStats.income
      ? Math.max(0, Math.round(((monthlyStats.income - monthlyStats.expense) / monthlyStats.income) * 100))
      : 0;
    const totalTransactions = monthlyStats.incomeCount + monthlyStats.expenseCount;
    const subscriptionCount = Array.isArray(subscriptions) ? subscriptions.length : 0;

    return [
      { ...numberData[0], value: String(totalTransactions) },
      { ...numberData[1], value: String(subscriptionCount) },
    ...
    <div className="stats-section-container">
          ...
          >
            ...
            <div>
              ...
              >
                ...
                {item.positive ? <TrendingUp className="arrow-icon" /> : <TrendingDown className="arrow-icon" />}
              </div>
            </div>
          </div>
      ...
      <div className="stats-right">
        {derivedNumbers.map((num, index) => (
          <div key={index} className="stats-number-card" style={{ opacity: loading ? 0.7 : 1 }}>
